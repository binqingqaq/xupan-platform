...
<template>
  <section :class="['player-desk-page', { 'player-desk-embedded': props.embedded }]">
    <header class="player-desk-header"><div><p class="eyebrow">ADMIN / PLAYER DESK</p><h1>玩家工作台</h1><p class="subline">普通玩家与托统一管理，托的行为只使用虚拟积分并留下动作记录。</p></div><div class="header-actions"><RouterLink v-if="!props.embedded" class="desk-link" to="/admin">返回管理台</RouterLink><button class="outline-button" type="button" @click="refresh()">刷新</button></div></header>
