...
public class TestPlayerBehaviorScheduler {
    ...
    public void dispatch() {
        ...
        jdbc.query("SELECT DISTINCT u.id FROM sys_user u JOIN demo_user_account a ON a.sys_user_id=u.id "
                        + "JOIN test_player_behavior b ON b.account_id=a.id WHERE b.enabled=TRUE AND a.player_kind='BOT' AND u.status='ACTIVE' AND a.status='ACTIVE'",
                rs -> { while (rs.next()) service.dispatch(rs.getLong(1)); });
    }
}
